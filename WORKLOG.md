...
**已知風險與待決（2026-09-02 討論）**：學員自訂產業鏈時**很可能不知道怎麼拆段**。AI 供應鏈的 16 個分類是顧老師拆好的，換成航運、金融，「拆段」變成學員自己的第一步，而段的定義是「同一個瓶頸位置上的同業」，不是上中下游的產業分類；只拆一段就沒有橫向比較，L1 到交集全部失去意義。目前編輯器新增鏈後只有一個空的「子段 1」，沒有任何引導。已提三個補法、等使用者決定：
1. **拆段前置指令**（非教材、需請顧老師過目）：讓學員問一般 AI「這條鏈依瓶頸位置可拆成哪幾段、每段台股上市櫃代號與名稱、每段租金來源」，要求輸出成控制台可直接「匯入分段設定」的 v2 格式，一鍵帶入子段與個股再自行刪改（同時解掉手打代號的麻煩）。
2. **編輯器內的拆段提示**：子段區上方寫拆段判準與一個航運示範（貨櫃／散裝／港埠，租金來源各異）；只有一個子段時顯示琥珀提示「一段沒有橫向比較可做，至少拆兩段」。
3. **講師下發優先**：月例會要跑的產業由講師先拆好、下發，學員套用後微調（功能已有，屬課堂流程安排）。
不建議內建航運、金融等預設模板——那等於替老師拆段，違反「先驗是老師的判斷」。1＋2 約半天工。
